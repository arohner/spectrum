...
;; (deftest invoke-predicate?
;;   (testing "truthy"
;;     (are [form] (= true (flow/invoke-predicate? (check/analyze-form form {:x (c/pred-spec #'any?)})))
;;       '(integer? x)))

;;   (testing "falsey"
;;     (are [form] (= false (flow/invoke-predicate? (check/analyze-form form {:x (c/pred-spec #'any?)})))
;;       '(inc x))))

;; (deftest infer-if-bindings
;;   (are [form args] (= args (:args (check/infer-form form)))
;;     '(fn [x] (inc x)) (:args inc-spec)
;;     '(fn [x] (if (integer? x) (inc x))) (c/cat- [(c/pred-spec #'any?)])
;;     '(fn [x y] (if (integer? x) (inc y))) (c/cat- [(c/pred-spec #'any?) (-> inc-spec :args c/first-)])))
